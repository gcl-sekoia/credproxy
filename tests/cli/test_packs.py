"""Pack placeholder validation -- now shared with injector parsing, so a bad
...
def _write_pack(name: str, placeholder_toml: str):
...
def test_pack_rejects_unknown_charset(xdg):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_pack("badcs", '[placeholder]\nprefix = "p_"\nlength = 32\ncharset = "nope"\n')
    with pytest.raises(ConfigError, match="charset"):
        load_packs()


def test_pack_rejects_zero_entropy_length(xdg):
    ...
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_pack("badlen", '[placeholder]\nprefix = "p_"\nlength = 2\ncharset = "hex"\n')
    with pytest.raises(ConfigError, match="must exceed"):
        load_packs()


def test_pack_valid_placeholder_builds(xdg):
    from credproxy_cli.core.model.packs import get_pack
    _write_pack("good", '[placeholder]\nprefix = "g_"\nlength = 18\ncharset = "hex"\n')
    ph = get_pack("good").placeholder.generate()
    assert ph.startswith("g_") and len(ph) == 18


def test_pack_placeholder_defaults_missing_fields(xdg):
    ...
    from credproxy_cli.core.model.packs import get_pack
    _write_pack("partial", '[placeholder]\nprefix = "x_"\n')   # no length/charset
    assert get_pack("partial").placeholder.prefix == "x_"


# ---- service setup packs: bindings + rules (#37) ----------------------------


def _write_raw_pack(name: str, toml: str):
    from credproxy_cli.core.paths import config_dir
    d = config_dir() / "packs"
    d.mkdir(parents=True, exist_ok=True)
...
def test_pack_mixed_parses_bindings_and_rules(xdg):
    from credproxy_cli.core.model.packs import build_pack, get_pack
    _write_raw_pack("gh-guarded", _MIXED)
    spec = get_pack("gh-guarded")
    assert spec.needs_credential is True
    exp = build_pack("gh-guarded", "env", "GITHUB_TOKEN")
...
def test_pack_pure_rule_needs_no_credential(xdg):
    """A pure-rule pack: no [placeholder], no [[part]] -> no provider/secret."""
    from credproxy_cli.core.model.packs import build_pack, get_pack
    _write_raw_pack("policy", _RULE_ONLY)
    spec = get_pack("policy")
    assert spec.needs_credential is False and spec.placeholder is None
    exp = build_pack("policy")                       # zero flags
    assert exp.bindings == ()
    assert [r.name for r in exp.rules] == ["policy-guard"] \
        and exp.rules[0].action == "block"


def test_pack_binding_without_placeholder_rejected(xdg):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_raw_pack("nop", '[[part]]\nsuffix="api"\ninjector="bearer"\n'
                             'hosts=["h.example"]\n')
    with pytest.raises(ConfigError, match="missing \\[placeholder\\]"):
        load_packs()


def test_pack_pure_rule_with_placeholder_rejected(xdg):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_raw_pack("bad", '[placeholder]\nprefix="p_"\n' + _RULE_ONLY)
    with pytest.raises(ConfigError, match="meaningless without"):
        load_packs()


def test_pack_empty_rejected(xdg):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_raw_pack("empty", '[placeholder]\nprefix="p_"\n')  # no parts, no rules
    with pytest.raises(ConfigError, match="at least one"):
        load_packs()


def test_pack_rule_literal_name_rejected(xdg):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import load_packs
    _write_raw_pack("bad", '[[rule]]\nname="x"\nsuffix="g"\n'
                             'hosts=["h.example"]\naction="block"\n')
    with pytest.raises(ConfigError, match="uses 'suffix'"):
        load_packs()


def test_pack_rule_field_error_surfaces_with_pack_path(xdg):
    """A bad pack rule fails at pack LOAD via the shared _parse_rule_entry,
    with the pack path in the message ONCE (not doubled -- review #38), not a
    ...
    with pytest.raises(ConfigError, match="action must be one of") as ei:
        load_packs()
    msg = str(ei.value)
    assert "pack 'bad'" in msg and "rule[0]" in msg
    assert msg.count("pack 'bad'") == 1        # not doubled


def test_describe_packs_includes_rules(xdg):
    from credproxy_cli.core.model.packs import describe_packs
    _write_raw_pack("gh-guarded", _MIXED)
    row = next(p for p in describe_packs() if p["name"] == "gh-guarded")
...
def test_pack_rejects_duplicate_setup_order(xdg):
...
def test_pack_rejects_duplicate_mount_target(xdg):
...
def test_pack_distinct_setup_orders_ok(xdg):
