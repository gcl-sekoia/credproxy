...
# setup.subprocess.run -- so a fake that re-invokes a real subprocess (see
...
def _make_binding_summary(name="b", injector="github", provider="env",
    ...
    from credproxy_cli.core.engine.containers import BindingSummary
...
def _capture_docker_args(monkeypatch):
    """Stub containers.docker.docker to record the args of each run. Also
    ...
    calls = []
    monkeypatch.setattr("credproxy_cli.core.engine.containers.docker.docker",
...
def test_workspace_disables_selinux_labeling(xdg, ws_factory, monkeypatch):
    ...
    containers.create_ws_container(ws, cfg, "deadbeef", proxy_id="pid")
    args = calls[-1]
...
def test_enrich_sysfs_with_keepid_adds_both_remedies(monkeypatch):
    ...
    monkeypatch.setattr("credproxy_cli.core.engine.runtime.is_podman_rootless", lambda: True)
    out = containers._enrich_ws_run_error(DockerError(_SYSFS_ERR), _keepid_cfg())
    msg = str(out)
...
def test_enrich_sysfs_without_keepid_map_host_user_off(monkeypatch):
    ...
    out = containers._enrich_ws_run_error(orig, _keepid_cfg(map_host_user=False))
    assert out is orig
...
def test_enrich_sysfs_with_run_flags_userns_override(monkeypatch):
    ...
    monkeypatch.setattr("credproxy_cli.core.engine.runtime.is_podman_rootless", lambda: True)
    orig = DockerError(_SYSFS_ERR)
    out = containers._enrich_ws_run_error(
...
def test_enrich_non_sysfs_failure_with_keepid(monkeypatch):
    ...
    out = containers._enrich_ws_run_error(orig, _keepid_cfg())
    assert out is orig
...
def test_emits_keep_id_predicate(monkeypatch):
    ...
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    monkeypatch.setattr("credproxy_cli.core.engine.runtime.is_podman_rootless", lambda: True)
    assert containers.emits_keep_id(_keepid_cfg()) is True
    assert containers.emits_keep_id(_keepid_cfg(map_host_user=False)) is False
    assert containers.emits_keep_id(_keepid_cfg(user="root")) is False
    assert containers.emits_keep_id(_keepid_cfg(run_flags=["--userns=host"])) is False
    # Non-rootless-podman: no keep-id emitted regardless.
    monkeypatch.setattr("credproxy_cli.core.engine.runtime.is_podman_rootless", lambda: False)
    assert containers.emits_keep_id(_keepid_cfg()) is False
...
def test_mount_parent_dirs_nested_yields_intermediate(xdg):
    from credproxy_cli.core.engine.containers import _mount_parent_dirs
...
def test_owns_user_mapping(xdg):
    from credproxy_cli.core.engine.containers import _credproxy_owns_user_mapping
...
def test_reserved_uid_check_rejects_user_uid(xdg):
    ...
    from credproxy_cli.core.engine.containers import _reserved_uid_check
...
def test_chown_mount_parents_uses_mapped_uid(xdg, ws_factory, monkeypatch):
    ...
    calls = _capture_docker_args(monkeypatch)
    containers.chown_mount_parents(ws, _nested_cfg(user_uid=1000), lambda *_: None)
    args = calls[-1]
...
def test_chown_mount_parents_falls_back_to_host_uid(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    containers.chown_mount_parents(ws, _nested_cfg(), lambda *_: None)  # no user_uid
...
def test_chown_user_owned_volumes_chowns_by_name(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    setup.chown_user_owned_volumes(ws, _uo_cfg(user="dev"), lambda *_: None)
    args = calls[-1]
    assert args[:4] == ["exec", "-u", "0", ws.ws_container]
...
def test_chown_user_owned_volumes_independent_of_map_host_user(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    setup.chown_user_owned_volumes(ws, _uo_cfg(map_host_user=False), lambda *_: None)
...
def test_chown_user_owned_volumes_noop_without_flag(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    setup.chown_user_owned_volumes(ws, _uo_cfg(user_owned=False), lambda *_: None)
    assert calls == []
...
def test_chown_user_owned_volumes_noop_root_or_no_user(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    setup.chown_user_owned_volumes(ws, _uo_cfg(user=None), lambda *_: None)
    setup.chown_user_owned_volumes(ws, _uo_cfg(user="root"), lambda *_: None)
    assert calls == []
...
def test_chown_mount_parents_noop_without_mapping(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a"); ws.ensure_state_dir()
    calls = _capture_docker_args(monkeypatch)
    containers.chown_mount_parents(ws, _nested_cfg(map_host_user=False), lambda *_: None)
    assert calls == []
...
def test_chown_mount_parents_noop_when_no_fabricated_parents(xdg, ws_factory, monkeypatch):
    ...
    cfg = _nested_cfg(mounts=[{"source": "x", "target": "/home/vscode/proj",
                               "readonly": False}])
    containers.chown_mount_parents(ws, cfg, lambda *_: None)
    assert calls == []
...
def test_drift_no_applied_record(xdg, workspaces_dir):
    from credproxy_cli.core.engine.containers import _compute_drift
...
def test_apply_container_drift_is_deferred(xdg, workspaces_dir, monkeypatch):
    ...
    from credproxy_cli.core.engine.startup import apply_config
    ...
    monkeypatch.setattr(
        "credproxy_cli.core.engine.containers.docker.resolve_host_port",
        lambda container, port: 39998,
    )
    monkeypatch.setattr(
        "credproxy_cli.core.engine.containers.ImageEnv.load",
    ...
    monkeypatch.setattr(
        "credproxy_cli.core.engine.startup.push_config",
...
""")
    ...
    monkeypatch.setattr("credproxy_cli.core.engine.containers.docker.container_status",
                        lambda name: "running")
    monkeypatch.setattr("credproxy_cli.core.engine.containers.docker.resolve_host_port",
                        lambda container, port: 39998)
    monkeypatch.setattr(
        "credproxy_cli.core.engine.containers.ImageEnv.load",
        classmethod(lambda cls: type("FakeEnv", (), {
...
def test_inspect_live_in_sync(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr(containers, "get_config", lambda url, token: {
...
def test_inspect_live_unavailable_when_proxy_unreachable(xdg, workspaces_dir, monkeypatch):
    ...
    _running_env(monkeypatch)
    monkeypatch.setattr(containers, "get_config", lambda url, token: None)
    data = containers.inspect_workspace(ws)
...
def test_apply_repushes_on_reality_drift_and_heals_doctor(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr(containers, "get_config", fake_get_config)
...
def test_apply_noops_when_live_in_sync(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr("credproxy_cli.core.engine.startup.push_config", fail_push)
    result = apply_config(ws)
    assert result.applied == ()
...
def test_count_live_sessions_empty(xdg, workspaces_dir):
    from credproxy_cli.core.engine.sessions import _count_live_sessions
...
def test_clean_stale_sessions(xdg, workspaces_dir):
    ...
    from credproxy_cli.core.engine.sessions import _clean_stale_sessions
...
def test_effective_config_resolves_enter_time_defaults(xdg):
    ...
    from credproxy_cli.core.engine.sessions import effective_config, DEFAULT_ENTER_PRELUDE
...
def test_effective_config_resolves_shell(xdg):
    ...
    from credproxy_cli.core.engine.sessions import effective_config, DEFAULT_ENTER_CMD
...
def test_run_setup_runs_every_call(xdg, ws_factory, monkeypatch):
    ...
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    calls = []
    monkeypatch.setattr(setup.subprocess, "run", _fake_run(calls))
    ws = ws_factory("a")
    cfg = {"setup": ["echo one", "echo two"]}
    setup.run_setup(ws, cfg, notify=lambda *_: None)
    setup.run_setup(ws, cfg, notify=lambda *_: None)
...
def test_run_setup_pins_root(xdg, ws_factory, monkeypatch):
    ...
    monkeypatch.setattr(setup.subprocess, "run", _fake_run(calls))
    ws = ws_factory("a")
    setup.run_setup(ws, {"setup": ["id -u"]}, notify=lambda *_: None)
    cmd = calls[0]
...
def test_run_setup_noop_without_commands(xdg, ws_factory, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(setup.subprocess, "run", _fake_run(calls))
    setup.run_setup(ws_factory("a"), {}, notify=lambda *_: None)
    assert calls == []
...
def test_run_setup_failure_raises(xdg, ws_factory, monkeypatch):
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    from credproxy_cli.core.errors import DockerError
    monkeypatch.setattr(setup.subprocess, "run", _fake_run([], code=7))
    with pytest.raises(DockerError):
        setup.run_setup(ws_factory("a"), {"setup": ["false"]},
...
def test_run_setup_string_entry_unchanged_argv(xdg, ws_factory, monkeypatch):
    ...
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    calls = []
    monkeypatch.setattr(setup.subprocess, "run", _fake_run_typed(calls))
    ws = ws_factory("a")
    setup.run_setup(ws, {"setup": ["echo hi"], "user": "vscode"},
...
def test_run_setup_execution_order(xdg, ws_factory, monkeypatch):
    ...
    setup.run_setup(ws, {"setup": setup_steps, "user": "vscode"})
...
def test_run_setup_home_resolved_per_step(xdg, ws_factory, monkeypatch):
    ...
    monkeypatch.setattr(setup.subprocess, "run", fake.run)
    ws = ws_factory("a")
    setup.run_setup(
        ws,
...
def test_resolve_home_numeric_user_via_fallback(xdg, ws_factory, monkeypatch, tmp_path):
    ...
    ws = ws_factory("a")
    assert setup._resolve_container_home(ws, "1000") == "/home/dev"
...
def test_resolve_home_dotted_user_literal(xdg, ws_factory, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(setup.subprocess, "run", _local_passwd_exec(
        "fooXbar:x:1001:1001::/home/fooXbar:/bin/sh\n", tmp_path))
    assert setup._resolve_container_home(ws, "foo.bar") is None
    # the literal name still resolves.
    monkeypatch.setattr(setup.subprocess, "run", _local_passwd_exec(
        "foo.bar:x:1002:1002::/home/foo.bar:/bin/sh\n", tmp_path))
    assert setup._resolve_container_home(ws, "foo.bar") == "/home/foo.bar"
...
def test_enter_push_flag_threads(xdg, ws_factory, monkeypatch):
    ...
    def fake_enter(ws, cmd, notify=None, user_override=None, push=False):
        captured["push"] = push
        return 0
    monkeypatch.setattr(sessions, "enter_workspace", fake_enter)
...
def test_setup_marker_and_retry(xdg, ws_factory):
    ...
    from credproxy_cli.core.engine.setup import _read_setup_marker, _setup_needed, _write_setup_marker
    ws = ws_factory("a")
...
class _StartFakeDocker:
    ...
    def inspect(self, target, fmt):
        from credproxy_cli.core.engine import containers, setup, startup, sessions
        if target == containers.IMAGE_TAG:
            return "imgid"
        if fmt == "{{.Id}}":
...
def _drive_start(monkeypatch, ws, fake, generation=7):
    ...
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    monkeypatch.setattr(containers.docker, "container_status", fake.container_status)
    monkeypatch.setattr(containers.docker, "docker", fake.docker)
    monkeypatch.setattr(containers.docker, "docker_quiet", fake.docker_quiet)
    monkeypatch.setattr(containers.docker, "inspect", fake.inspect)
    monkeypatch.setattr(containers.docker, "resolve_host_port", fake.resolve_host_port)
    monkeypatch.setattr(startup, "wait_for_ready", lambda port: None)
    monkeypatch.setattr("credproxy_cli.core.engine.containers.ImageEnv.load",
...
def test_lifecycle_lock_is_sole_applied_baseline(xdg, workspaces_dir, monkeypatch):
    ...
    setup_runs: list = []
    monkeypatch.setattr(setup, "run_setup",
...
def test_start_proxy_image_change_removes_workspace_before_proxy(xdg, workspaces_dir,
    ...
    calls: list = []
    monkeypatch.setattr(containers.docker, "container_status", lambda name: "running")
    ...
    monkeypatch.setattr(containers.docker, "inspect", fake_inspect)
    monkeypatch.setattr(containers.docker, "docker_quiet",
                        lambda argv: calls.append(("quiet", argv)))
    monkeypatch.setattr(containers.docker, "docker",
    ...
    monkeypatch.setattr(containers, "create_proxy", boom)
...
def _stub_recreate_deps(monkeypatch):
    ...
    monkeypatch.setattr(startup, "start_workspace",
                        lambda ws, notify=containers._noop: started.append(ws.name))
...
def test_recreate_proxy_removes_both(xdg, workspaces_dir, monkeypatch):
    ...
    startup.recreate_workspace(ws, include_proxy=True)
...
def test_recreate_reset_volume_drops_after_container(xdg, workspaces_dir,
    ...
    startup.recreate_workspace(ws, reset_volumes=["home", "cache"])
...
def test_recreate_reset_unknown_volume_rejected(xdg, workspaces_dir, monkeypatch):
    ...
    with pytest.raises(ConfigError, match="hmoe"):
        startup.recreate_workspace(ws, reset_volumes=["hmoe"])   # typo of "home"
...
def test_delete_removes_workspace_volumes(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr(containers.docker, "docker_output", fake_output)
    rm: list = []
    monkeypatch.setattr(containers.docker, "docker_quiet", lambda argv: rm.append(argv))

    containers.delete_workspace(ws)
...
def test_delete_keep_volumes(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr(containers.docker, "docker_output",
                        lambda argv: ws.volume("home"))
    rm: list = []
    monkeypatch.setattr(containers.docker, "docker_quiet", lambda argv: rm.append(argv))

    containers.delete_workspace(ws, keep_volumes=True)
...
def test_managed_volumes_created_with_owner_label(xdg, ws_factory, monkeypatch):
    ...
    ws = ws_factory("a")
    calls: list = []
    monkeypatch.setattr(containers.docker, "docker_quiet", lambda argv: calls.append(argv))
    cfg = {"mounts": [
    ...
    containers._ensure_managed_volumes(ws, cfg)
    assert calls == [
...
def test_workspace_volumes_label_isolates_name_prefix_siblings(xdg, workspaces_dir):
    ...
    from credproxy_cli.core.engine import docker, containers, setup, startup, sessions
    from credproxy_cli.core.errors import DockerError
    try:
    ...
    try:
        containers._ensure_managed_volumes(foo, cfg)
        containers._ensure_managed_volumes(foobar, cfg)
        # foo's enumeration sees only foo's volume, though its NAME prefix
        # (credproxy-vol-<base>-) also prefixes foo-bar's volume name.
        assert containers._workspace_volumes(foo) == [foo.volume("home")]
        assert containers._workspace_volumes(foobar) == [foobar.volume("home")]
    finally:
...
def test_add_managed_volume_plain_edits_toml_only(xdg, workspaces_dir, monkeypatch):
    ...
    monkeypatch.setattr(containers.docker, "container_status",
                        lambda c: (_ for _ in ()).throw(AssertionError("no docker")))
    monkeypatch.setattr(startup, "recreate_workspace",
                        lambda *a, **kw: (_ for _ in ()).throw(AssertionError("no recreate")))
    startup.add_managed_volume(ws, name="cache", target="/c",
...
def test_add_managed_volume_preserve_ordering(xdg, workspaces_dir, monkeypatch):
    ...
    ws = _write_ws(workspaces_dir, "w")
    events = []
    monkeypatch.setattr(containers.docker, "container_status", lambda c: "running")
    monkeypatch.setattr(containers.docker, "docker_quiet",
    ...
    monkeypatch.setattr(containers.docker, "seed_volume_from_container", _seed)
    monkeypatch.setattr(startup, "recreate_workspace",
...
def test_add_managed_volume_preserve_rollback_on_capture_failure(
    ...
    monkeypatch.setattr(containers.docker, "seed_volume_from_container",
...
def test_add_managed_volume_preserve_requires_container(xdg, workspaces_dir, monkeypatch):
    ...
    from credproxy_cli.core.errors import WorkspaceError
    ws = _write_ws(workspaces_dir, "w")
    monkeypatch.setattr(containers.docker, "container_status", lambda c: None)
...
def test_add_managed_volume_home_uses_sugar(xdg, workspaces_dir):
    ...
    from credproxy_cli.core.engine import containers, setup, startup, sessions
    ws = _write_ws(workspaces_dir, "w")
    startup.add_managed_volume(ws, name="home", target="/home/vscode",
                                 readonly=False, preserve=False)
    raw = tomllib.loads(ws.config_path.read_text())
...
def test_reload_proxy_waits_for_ready(monkeypatch, ws_factory):
    ...
    ws = ws_factory("r")
    events = []
    monkeypatch.setattr(containers.docker, "container_status", lambda n: "running")
    monkeypatch.setattr(containers.docker, "docker",
                        lambda args, **k: events.append(("docker", args)))
    monkeypatch.setattr(containers.ImageEnv, "load",
                        classmethod(lambda cls, image=None: ImageEnv(
                            http_port=39998, tmpfs="/t", token="/tok",
                            source="/opt/proxy", mitmproxy_uid=31337)))
    monkeypatch.setattr(containers.docker, "resolve_host_port",
                        lambda c, p: 54321)
    monkeypatch.setattr(startup, "wait_for_ready",
