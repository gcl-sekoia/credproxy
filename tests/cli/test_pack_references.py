"""config-v2 (#63): packs are durable `[[pack]]` REFERENCES; the resolver
...
to packs, `disable` / `[pack.override.*]`, unknown-suffix errors, the lock
schema, and literal-then-pack merge ordering + collisions.
"""
...
def test_reference_expands_and_snapshots_lock(xdg, workspaces_dir):
    ...
    # Literal-then-pack: no literal here, so just the pack expansion.
...
def test_literal_then_pack_merge_order(xdg, workspaces_dir):
    from credproxy_cli.core.model.resolver import resolve_workspace
    _pack("github", _GH)
    ws = _ws(workspaces_dir, "w", """
        image = "x"
...
def test_definition_change_note_surfaces_on_stderr(xdg, workspaces_dir):
    ...
    via a mutating verb -- here `pack add` (which resolves for the real action).
    ...
    code, out, err = _run(["workspace", "w", "pack", "add", "policy"])
...
def test_disable_enable_cycle_keeps_placeholder(xdg, workspaces_dir):
    ...
    r1 = resolve_workspace(ws)
    save_lock(ws, r1.lock)
    ph0 = r1.lock["packs"]["github"]["placeholder"]
    ...
    assert r2.lock["packs"]["github"]["placeholder"] == ph0
    ...
    assert r3.lock["packs"]["github"]["placeholder"] == ph0
...
def test_deleted_pack_with_edited_inputs_still_errors(xdg, workspaces_dir):
    ...
    ws.config_path.write_text(
        'image = "x"\n[[pack]]\nname = "acme"\ndisable = ["git"]\n')
    with pytest.raises(CredproxyError, match="unknown pack"):
        resolve_workspace(ws)


def test_override_unknown_binding_field_errors(xdg, workspaces_dir):
    """A `[pack.override.<suffix>]` on a BINDING that names an unknown field
...
def test_pack_vs_literal_binding_collision_names_both(xdg, workspaces_dir):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.resolver import resolve_workspace
    _pack("github", _GH)
    ws = _ws(workspaces_dir, "w", """
...
def test_unknown_key_under_pack_rejected(xdg, workspaces_dir):
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.resolver import resolve_workspace
    _pack("github", _GH)
    ws = _ws(workspaces_dir, "w",
...
def test_duplicate_reference_rejected(xdg, workspaces_dir):
    ...
    ws = _ws(workspaces_dir, "w",
             'image = "x"\n[[pack]]\nname = "github"\n[[pack]]\nname = "github"\n')
    with pytest.raises(ConfigError, match="duplicate `\\[\\[pack\\]\\]` reference"):
        resolve_workspace(ws)
...
def test_multislot_reference_expands_with_table_secret(xdg, workspaces_dir):
    ...
    assert r.lock["packs"]["aws"]["inputs"]["secret"] == {
...
def test_multislot_secret_slot_mismatch_rejected(xdg, workspaces_dir):
    ...
    the whole expansion (atomic), pack-framed."""
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.resolver import resolve_workspace
    _pack("aws", _AWS)
    ws = _ws(workspaces_dir, "w", """
...
def test_parts_with_divergent_slot_sets_rejected(xdg, workspaces_dir):
    ...
    expansion."""
    from credproxy_cli.core.errors import ConfigError
    from credproxy_cli.core.model.packs import pack_slot_set, get_pack
    _pack("mixed", """
        [placeholder]
        prefix = "m_"
        length = 20
