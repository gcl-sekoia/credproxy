"""Tests for `exec` (lifecycle._exec_cmd argv modes + exec_workspace behavior)."""
...
def test_exec_registers_pidfile_but_never_auto_stops(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lifecycle, "load_config", lambda ws: {"user": "dev"})
    monkeypatch.setattr(lifecycle, "_maybe_auto_stop",
    ...
    rc = lifecycle.exec_workspace(ws, ["true"], mode="raw")
    assert rc == 0
...
def test_exec_maps_signal_death_exit_code(tmp_path, monkeypatch):
    ...
    ws = _fake_ws(tmp_path)
    monkeypatch.setattr(lifecycle, "_start_for_exec", lambda *a, **k: None)
    monkeypatch.setattr(lifecycle, "load_config", lambda ws: {})
    monkeypatch.setattr(subprocess, "run",
                        lambda *a, **k: types.SimpleNamespace(returncode=-signal.SIGINT))
    rc = lifecycle.exec_workspace(ws, ["sleep", "9"], mode="raw")
...
def test_exec_fast_path_skips_start_when_both_running(tmp_path, monkeypatch):
    ...
    lifecycle._start_for_exec(ws, lifecycle._noop, push=False)
...
def test_exec_fast_path_bypassed_by_push(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lifecycle, "start_workspace",
                        lambda *a, **k: called.__setitem__("start", True))
    lifecycle._start_for_exec(ws, lifecycle._noop, push=True)
