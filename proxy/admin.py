...
generated by `make up` if absent) and bind-mounted read-only into the
proxy. Config lives on tmpfs at /run/secrets/config.json -- survives
SIGHUP-triggered python re-exec (`make reload`); re-pushed by the host
CLI after a full container restart.
"""
import hmac
import json
import os
