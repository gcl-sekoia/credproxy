...
def test_claude_code_is_the_umbrella_enablement_pack():
    ...
    # Ships the session runner + its default fragments (sandbox, credproxy, config-dir, tools).
    targets = {m.target for m in exp.mounts}
    assert {"/opt/session-context.sh", "/opt/session-context.d/05-environment.sh",
            "/opt/session-context.d/10-credproxy.sh",
