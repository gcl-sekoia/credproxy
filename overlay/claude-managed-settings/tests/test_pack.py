...
In the full overlay chain 50-example shadows this pack, so `get_pack` /
`build_pack` return the shadow; to check THIS overlay's own neutral default we
parse its own file directly.
"""
from pathlib import Path

from credproxy_cli.core.model.packs import _parse_pack

PACK = Path(__file__).resolve().parent.parent / "packs" / "claude-managed-settings.toml"


def test_pack_parses_and_is_a_neutral_noop():
    spec = _parse_pack(PACK, "claude-managed-settings", "claude-managed-settings")
