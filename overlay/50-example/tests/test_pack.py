"""The 50-example overlay's packs: its own `persist`, and its SHADOW of
...
from credproxy_cli.core.model.packs import build_pack, load_pack_sources
...
def test_claude_managed_settings_shadow_provides_the_aggressive_patch():
    ...
    assert load_pack_sources().get("claude-managed-settings") == "overlay:50-example"
    exp = build_pack("claude-managed-settings")
    assert exp.bindings == ()
    (rule,) = exp.rules
