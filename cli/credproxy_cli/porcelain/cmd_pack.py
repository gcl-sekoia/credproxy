"""The `pack` noun's workspace-scoped verbs (add/refresh/remove) plus the shared
pack-reference helpers: option/credential resolution, block rendering, the
...
# The `[[pack]]` header + `[pack.<child>]` sub-table regexes are the SINGLE
# whitespace-tolerant source in `core.model.packs` (shared with `remove_pack`
...
from ..core.model.packs import (
    _PACK_HEADER_RE as _PACK_BLOCK_RE,
    _PACK_CHILD_RE,
)
# A `[[pack]]` header OR an inline `pack =` assignment, for detecting pack
# references in a template we can't fully parse.
_PACK_REF_RE = re.compile(r"^\s*(\[\[\s*pack\s*\]\]|pack\s*=)", re.M)


def _newly_intercepted(existing_hosts, new_hosts) -> list[str]:
    """Hosts the pack newly flips to TLS-intercepted: `new_hosts` not already
    ...
    (the UNION intercept set), so `pack add` announces this rather than letting
...
def _render_pack_ref_block(name: str, provider, secret, options: dict) -> str:
    """Render a `[[pack]]` reference block (leading blank line) with the RESOLVED
    provider/secret/options written explicitly -- what `pack add` / `create`
    ...
    lines = ["", "[[pack]]", f"name     = {_toml_str(name)}"]
...
def _append_pack_ref(text: str, name: str, provider, secret,
                       options: dict) -> str:
    """Append a `[[pack]]` reference block at EOF (array-of-tables merge in file
    order, like `[[binding]]`)."""
    if text and not text.endswith("\n"):
        text += "\n"
    return text + _render_pack_ref_block(name, provider, secret, options)


def _expansion_summary(name: str, entry: dict) -> dict:
    """Render-ready announce dict for a minted pack snapshot (`entry` is the
    lock's `packs[name]`). Reads the intent-level `expansion` dicts."""
    exp = entry["expansion"]
...
def _resolve_pack_option_values(ctx: Ctx, spec, explicit: dict) -> dict:
    """Resolve every pack `[[option]]` to a value in the settled order (#59):
    explicit (`--opt`/template `[pack.options]`) -> prompt (loose+TTY only) ->
    default -> structured fail. Returns `{id: value}` (empty for an option-less
    pack). Raises `PackOptionsError` (structured under `--json`) when a required
    ...
    if not spec.options:
        ...
        if not explicit:
            return {}
    from ..core.errors import PackOptionsError
    from ..core.model.packs import option_summary, resolve_options
    ...
    if missing:
        raise PackOptionsError(spec.name, [option_summary(o) for o in missing])
    return values


def _resolve_pack_credential_interactive(
    ...
    from ..core.model.packs import pack_slot_set, resolve_pack_credential
    from . import prompt as prompt_mod
    ...
    if missing and prompt_mod.prompting_enabled(ctx):
        ...
        if "secret" in missing and pack_slot_set(spec) == ("value",):
...
def do_pack_add(ctx: Ctx, name: str | None, a: argparse.Namespace) -> None:
    """Apply a pack as a service setup pack: append a durable `[[pack]]`
    ...
    proxy never sees a "pack"; the resolver expands the reference. A pure-rule /
    pure-container pack needs no provider/secret."""
    from ..core.model.packs import (
        apply_option_values, get_pack, parse_pack_refs, pack_slot_set,
    )
    from ..core.providers import find_provider

    spec = get_pack(a.pack)               # CredproxyError -> clean fail on unknown
    ...
    if spec.needs_credential:
        ...
        def _missing(missing):
            if "provider" in missing:
                fail("pack '%s' has bindings but no default provider -- pass "
                     "--provider" % a.pack)
            if slots == ("value",):
                fail("`pack add` needs --secret (its meaning depends on "
                     "--provider)")
            fail("`pack add` needs `--secret SLOT=REF` for each slot: "
                 f"{', '.join(slots)}")
        ...
        fail(f"pack '{a.pack}' is a {shape} pack with no bindings -- it "
    ...
    with ws.lock():                          # atomic read-validate-write
        ...
        # Double-add guard: a `[[pack]]` reference for this pack already present.
        if a.pack in {r.name for r in parse_pack_refs(tomllib.loads(text), source)}:
            fail(f"pack '{a.pack}' is already referenced here "
                 f"(a `[[pack]]` block names it); edit that block instead")
        ...
        if attached and spec.has_container_half:
            fail(f"pack '{a.pack}' carries container-half config "
        ...
        atomic_write_text(
            ws.config_path,
            _append_pack_ref(text, a.pack, provider, secret, option_values))
        try:
    ...
    entry = resolved.lock["packs"][a.pack]
    announce = _expansion_summary(a.pack, entry)
    ...
    # `pack add` is otherwise a pure config edit, so a missing/unreachable
    ...
    requires = [prereqs.summary(r) for r in prereqs.evaluate(
        ...
        do_fetch=True)]
    render.OUT.pack_applied(
        ws.name, a.pack, announce["bindings"], announce["rules"],
...
def do_pack_refresh(ctx: Ctx, name: str | None, a: argparse.Namespace) -> None:
    """Re-expand the workspace's `[[pack]]` reference(s) from their CURRENT
    ...
    from ..core.errors import CredproxyError
    from ..core.model import pack_refresh
    from ..core.model.lock import save_lock
    from ..core.model.resolver import resolve_workspace
    ...
    with ws.lock():
        ...
        result = pack_refresh.compute_refresh(ws, a.pack)  # a.pack may be None
        ...
        if not a.check and result.changed:
            _confirm_destructive(ctx, ws, implicit, "refresh packs of")
    ...
    render.OUT.pack_refreshed(
        ws.name, [p.to_dict() for p in result.packs], check=a.check,
        newly_intercepted=newly,
        recreate=(container_exists and result.container_half_changed),
        attached=attached, written=written)


def do_pack_remove(ctx: Ctx, name: str | None, a: argparse.Namespace) -> None:
    """Remove a `[[pack]]` reference block (+ its `[pack.options]`/
    `[pack.override.*]` child sub-tables) and drop its lock snapshot. Reports
    what leaves the effective model + whether hosts stop being intercepted."""
    from ..core.errors import CredproxyError
    from ..core.model.lock import load_lock
    from ..core.model.packs import parse_pack_refs, remove_pack
    from ..core.model.resolver import resolve_workspace
    ...
    # The ONLY hard preconditions are: the pack is referenced (`parse_pack_refs`
    # needs no pack definition, so a dangling ref still parses) + the destructive
    # gate. Everything downstream is best-effort REPORTING -- `pack remove` must
    # succeed exactly when removal is the fix (a literal-vs-pack collision, or a
    ...
    refs = {r.name for r in parse_pack_refs(
        tomllib.loads(ws.config_path.read_text()), str(ws.config_path))}
    if a.pack not in refs:
        fail(f"pack '{a.pack}' is not referenced in workspace '{ws.name}'")

    _confirm_destructive(ctx, ws, implicit, "remove pack from")
    ...
    # the fix).
    entry = load_lock(ws).get("packs", {}).get(a.pack)
    summary = _expansion_summary(a.pack, entry) if entry else {
    ...
    render.OUT.pack_removed(
        ws.name, a.pack, summary["bindings"], summary["rules"], no_longer,
